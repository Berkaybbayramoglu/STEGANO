...
def main() -> None:
    ...
    if args.sweep and Path(args.sweep).exists():
        ...
        # Extract mean performance at 0.153 bpp (10,000 bit) for comparison to eliminate cherry-picking
        ref = df_sweep[df_sweep["Payload_Bit"] == 10_000]
        sweep_row = None
        sweep_stats = None
        if not ref.empty:
            # Calculate arithmetic means across the entire subset
            sweep_row = {
                "psnr": ref["PSNR_dB"].mean(),
                "ssim": ref["SSIM"].mean(),
                "rs":   ref[COL_RS].mean(),
            ...
            sweep_stats = {
                "mean": {
                    "PSNR_dB": ref["PSNR_dB"].mean(),
                    "SSIM": ref["SSIM"].mean(),
                    "RS_stat": ref[COL_RS].mean(),
                    "chi2_p": ref[COL_CHI2P].mean(),
                },
                "std": {
                    "PSNR_dB": ref["PSNR_dB"].std(),
                    "SSIM": ref["SSIM"].std(),
                    "RS_stat": ref[COL_RS].std(),
                    "chi2_p": ref[COL_CHI2P].std(),
                }
            }
            print(f"  Mean 10,000-bit: PSNR={sweep_row['psnr']:.2f}  "
                  f"SSIM={sweep_row['ssim']:.4f}  RS={sweep_row['rs']:.5f}  chi2_p={sweep_row['chi2']:.5f}")
        plot_method_comparison(out_dir, sweep_row)
        generate_radar_comparison(out_dir, sweep_stats)
        generate_stability_line(out_dir, df_sweep)
